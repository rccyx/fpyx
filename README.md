...
FNV-1a (Fowler-Noll-Vo) is a simple, fast, non-cryptographic hash function designed for hash tables and checksums.

**Pros:**

- Fast: ~0.5 microseconds for typical payloads
- Tiny: ~10 lines of code, pure arithmetic (XOR and multiplication)
- Deterministic: Same input always produces same output
- Good distribution: Low collision rate for typical use cases
- No dependencies: No crypto APIs, no external libraries

**Cons:**

- Not cryptographic: Do not use for passwords, secrets, or data integrity
- 64-bit output: Shorter than SHA-256. Collisions are more likely (but acceptable for rate limiting)
- No keying: Not suitable for message authentication (use HMAC for that)

**When to use:**

Rate limiting keys, cache keys, hash tables, checksums, non-security-critical identifiers.

**When not to use:**

Passwords, authentication tokens, digital signatures, data integrity verification.
...
- ~10–20× faster than JSON parsing for small objects
...
- Trust only IP headers your proxy sets
- Configure your proxy to overwrite client-supplied headers
- Enforce limits server-side
- Return HTTP 429 when limits are exceeded
- Use path normalization to collapse IDs
- Test your limits under load
- Monitor rate limit hits and adjust thresholds

**Don't:**

- Trust client-supplied IP headers directly
- Use fingerprints for identity or authentication
- Expect fingerprints to be unique per user
- Use FNV-1a for cryptographic purposes
- Forget to set TTLs on your rate limit keys
...
[Fastly Client IP](https://developer.fastly.com/reference/http/http-headers/Fastly-Client-IP/)

[Fly.io Client IP](https://fly.io/docs/reference/runtime-environment/#fly-client-ip)

[Akamai True-Client-IP](https://techdocs.akamai.com/origin-ip-acl/docs/true-client-ip)
